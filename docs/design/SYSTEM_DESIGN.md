...
the client layer is stubbed with an OpenAPI 3.1 contract, curl examples and a Go integration harness.
...
| Error code vocabulary beyond the two DRAFT names | DRAFT §8 gives `VIN_INVALID` and `ALL_SOURCES_UNAVAILABLE`; `UPSTREAM_TIMEOUT` and `UPSTREAM_ERROR` added for per-source reporting under FR7 |
...
| More than one service instance | Replace the `CacheStore` adapter with Redis; the port already exists |
...
| Client layer stubbed | OpenAPI 3.1, curl examples, Go harness | Harness runs against both mock servers |
