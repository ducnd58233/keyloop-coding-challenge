...
| **T2** | Scaffold + walking skeleton                             | A8, K5, R1-R6                                     | Module tree per SPEC §5 with both slices stubbed; `make help` and `make tools` both work; `make infra-up` brings up PostgreSQL 17 with a healthcheck, and `make dev` (infra + migrate + mocks + API) works from a clean clone; `configs` **package at the repository root is the only reader of environment variables**, and `Load()` **fails fast if the timeout ordering in SPEC §6 is violated**; `internal/<service>/app` is the composition root for each binary; `go run ./cmd/documentviewer` serves `/healthz` and `/readyz`; every target listed by `make help` runs; graceful shutdown | S    |
