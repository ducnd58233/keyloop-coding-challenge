...
| **T8** | Contract + README                                       | Deliverable 2                                     | `make openapi` regenerates `api/<service>/http/docs/` from handler annotations and `make openapi-check` passes; live `/docs` + `/openapi.json` on each binary; README covers `stack-up`/`stack-down` order, OpenAPI URLs, seed VINs, commands, **and the AI Collaboration Narrative**                                                                                                                                                                                                                                                                                                                                                                  | M    |
...
- [x] **T6** - HTTP layer
- [ ] T7 - Observability
- [x] **T8** - Contract and README (`feature/doc-viewer-task-8-contract-readme`)
- [ ] T9 - Demo rehearsal

**Not blocked.** T8 serves `/docs` + `/openapi.json` and documents `stack-up` / `stack-down`. T7 is next.
